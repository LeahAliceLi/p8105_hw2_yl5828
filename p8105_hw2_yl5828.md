...
was **0**. However, we could notice that this 0 is from the new merged
datasheet. If we are using the original Gwynnda dataset, the number of
cigarette butts will not be 0, the actual number would be 18120.

The dataset contains **1016** observations of trash collection events
...
`gwynnda_trash_df` include **192** dumpsters. The combined dataset has
...
```
    ...
    ## Warning in left_join(zori_clean, zip_codes_clean, by = c(zip = "zip_code")): Detected an unexpected many-to-many relationship between `x` and `y`.
